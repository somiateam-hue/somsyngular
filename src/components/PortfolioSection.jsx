...
function PortfolioCard({ item, onClick }) {
  ...
  return (
    <div 
      ...
      className="portfolio-card group relative cursor-pointer rounded-[1.75rem] overflow-hidden bg-black/40 border border-white/10 hover:border-accent-neon/30 transition-colors duration-300"
    >
      {/* Media area */}
      ...
      <div className="max-w-7xl mx-auto space-y-14">
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(item => (
              <PortfolioCard key={item.id} item={item} onClick={setSelectedCase} />
            ))}
          </div>
        ) : (
