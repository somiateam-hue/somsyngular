...
import { ArrowRight, BrainCircuit, Zap, Shield } from "lucide-react";
...
const Button = ({ variant = "default", size = "sm", className = "", children, ...props }) => {
  ...
  const variants = {
    default: "bg-purple-600 text-white hover:bg-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.3)] hover:shadow-[0_0_30px_rgba(168,85,247,0.5)]",
    outline: "border border-white/15 bg-white/5 backdrop-blur-xl text-white hover:bg-white/10 hover:border-purple-500/40",
...
export default function EtherealBeamsHero() {
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black">
      ...
      {/* Hero Content */}
      <div className="relative z-10 flex min-h-[calc(100vh-4rem)] items-center pt-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
          <div className="mx-auto max-w-4xl text-center">
            {/* Badge */}
            <div className="mb-8 inline-flex items-center rounded-full bg-white/5 backdrop-blur-xl border border-purple-500/20 px-4 py-2 text-sm text-white/80">
              <Zap className="mr-2 h-4 w-4 text-purple-400" />
              Automatización e Inteligencia Artificial
            </div>

            {/* Main Heading */}
            <h1 className="mb-6 text-[2.5rem] leading-[1.1] sm:leading-tight font-bold tracking-tight text-white sm:text-6xl lg:text-7xl font-display">
            ...
            <p className="mb-10 text-lg leading-8 text-white/70 sm:text-xl max-w-2xl mx-auto">
              Necesita un <strong className="text-white">sistema inteligente</strong>. Automatización avanzada con IA
              al servicio de tus resultados — desde el día uno.
            </p>

            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
              <Button size="lg" className="w-full sm:w-auto font-semibold" onClick={() => document.querySelector('#soluciones')?.scrollIntoView({ behavior: 'smooth' })}>
                <BrainCircuit className="mr-2 h-5 w-5" />
                Ver Soluciones
                <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
              </Button>
              <Button variant="outline" size="lg" className="w-full sm:w-auto font-semibold" onClick={() => document.querySelector('#proceso')?.scrollIntoView({ behavior: 'smooth' })}>
                Cómo funciona
              </Button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-6 max-w-lg mx-auto">
              {[
                ...
                { icon: BrainCircuit, value: "IA", label: "Nativa" },
                { icon: Shield, value: "24/7", label: "Operativo" },
              ].map(({ icon: Icon, value, label }) => (
                <div key={label} className="text-center">
                  <div className="flex items-center justify-center mb-1">
                    <Icon className="h-4 w-4 text-purple-400 mr-1" />
      ...
      <div className="absolute inset-0 z-0 bg-gradient-to-t from-black/70 via-black/20 to-black/40 pointer-events-none" />
    </div>
  );
}
